...
let templateStr = '';
try {
    let resPath = path_1.default.join(__dirname, 'PluginUserIndex.liquid');
...
try {
    const metaData = JSON.parse(fs_extra_1.default.readFileSync(path_1.default.resolve(__dirname, 'PluginUserIndexMetaData.json')).toString('utf-8'));
    ...
    fs_extra_1.default.writeFileSync(path_1.default.resolve(__dirname, 'UserIndex.js'), script);
}
catch (err) {
    if (err instanceof Error) {
        console.error(err.message);
